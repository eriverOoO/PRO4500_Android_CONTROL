...
6. 앱은 `Auto focus` ON으로 시작합니다. 촬영 전에 피사체를 보면서 초점이 맞을 때까지 기다린 뒤 `Auto focus`를 OFF로 바꾸면 그 순간의 렌즈 위치가 그대로 고정됩니다. 촬영 명령 시 아직 ON이면 안전을 위해 현재 초점으로 고정하고 OFF로 전환합니다.
7. 0도 촬영 중에는 OFF 상태를 유지합니다. 180도 회전 후 촬영 대기 중 `Auto focus`를 ON으로 풀어 다시 맞춘 다음 OFF로 고정하고 `Next Angle`로 촬영을 계속합니다. 앱이 촬영 종료 후 임의로 AF를 다시 켜지 않습니다.
8. 화면 터치, `AF Once`, `Lock AF`도 초점을 맞춘 뒤 OFF 고정하며, 다시 자동으로 맞추려면 `Auto focus`를 ON으로 바꾸거나 `Resume AF`를 누릅니다.
9. 정확한 디옵터 값을 직접 지정하려면 `Manual focus distance`를 켜고 `Focus` 값을 입력한 뒤 `Apply`를 누릅니다. `0.0 D`는 무한대이며 가까운 피사체에는 흐릴 수 있습니다.
