...
6. 기본 초점 흐름은 `Auto focus` ON입니다. 미리보기에서는 연속 AF가 동작하고, 스캔 첫 촬영 직전에 중앙 AF를 한 번 수행한 뒤 전체 스캔 동안 같은 렌즈 위치로 고정합니다. 마지막 프레임 업로드 후 연속 AF가 다시 시작됩니다.
7. 촬영 전에 직접 고정하려면 `Auto focus`를 OFF로 바꾸거나 `AF Once`, `Lock AF`, 미리보기 화면 터치를 사용합니다. 다시 자동으로 맞추려면 `Auto focus`를 ON으로 바꾸거나 `Resume AF`를 누릅니다.
8. 정확한 디옵터 값을 직접 지정하려면 `Manual focus distance`를 켜고 `Focus` 값을 입력한 뒤 `Apply`를 누릅니다. `0.0 D`는 무한대이며 가까운 피사체에는 흐릴 수 있습니다.
